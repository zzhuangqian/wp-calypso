...
/* TODO: No state? Make stateless component? */
class ProductImageUploader extends Component {
	...
	onPick = ( files ) => {
		...
		images.forEach( function( image ) {
			...
			previews[ transientId ] = URL.createObjectURL( image );
		} );
			...
			if ( ! isUploadInProgress ) {
				if ( media ) {
					onUpload( {
						...
						transientId,
						URL: media.URL,
						placeholder: previews[ transientId ],
					} );
	...
	render() {
		...
		const uploader = compact && this.renderCompactUploader() || this.renderUploader();
		return (
			<div>
				{ uploader }
			</div>
...
export default connect( null, { errorNotice: errorNoticeAction } )( localize( ProductImageUploader ) );
